...
import { User, FileText, Calendar, Users, TrendingUp } from 'lucide-react';
...
const SenatorCard: React.FC<SenatorCardProps> = ({ senator, compact = false }) => {
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1">
              ...
              <div className="flex items-center gap-2 text-sm">
                <span className={`px-2 py-0.5 rounded-full text-xs ${getPartyColor(senator.party)}`}>
                  {senator.party}
                </span>
                {senator.position && (
            ...
            <div className="text-right">
              <p className="text-sm text-gray-600">{senator.stats.billsPassed} bills passed</p>
              <p className="text-xs text-gray-500">{senator.stats.attendanceRate}% attendance</p>
            </div>
          </div>
        </Card>
      </Link>
        ...
        <div className="p-6">
          ...
          <div className="flex items-start justify-between mb-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-lg font-semibold text-gray-900">{senator.name}</h3>
                {senator.position && (
          ...
          {/* Party Badge */}
          <div className="mb-4">
            <span className={`inline-block px-3 py-1 rounded-full text-sm font-medium ${getPartyColor(senator.party)}`}>
              {senator.party}
            </span>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4 mb-4">
            <div>
              ...
              <p className="text-2xl font-bold text-gray-900">{senator.stats.billsAuthored}</p>
            </div>
            <div>
              <div className="flex items-center gap-1 text-gray-600 mb-1">
              ...
              <p className="text-2xl font-bold text-green-600">{senator.stats.billsPassed}</p>
            </div>
          </div>
          ...
          <div className="mb-4">
            <div className="flex justify-between text-sm mb-1">
              ...
              <span className="font-medium">{senator.stats.attendanceRate}%</span>
            </div>
            ...
            <div className="border-t pt-4">
              ...
              <div className="flex flex-wrap gap-1">
                {senator.committees.slice(0, 2).map(committee => (
                  ...
                  >
                    {committee.role === 'Chairperson' ? 'Chair' : committee.role}: {committee.name.split(' ').slice(0, 3).join(' ')}
                  </span>
                ))}
          ...
          <div className="mt-4 pt-4 border-t flex items-center justify-between text-xs text-gray-500">
            <span>Term: {new Date(senator.termStart).getFullYear()} - {new Date(senator.termEnd).getFullYear()}</span>
