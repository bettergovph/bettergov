...
import { TrendingUp, TrendingDown, Activity, Award, Target, Users } from 'lucide-react';
import { Card } from '../ui/Card';
import { Senator, Bill } from '../../types/senate';
...
const PerformanceMetrics: React.FC<PerformanceMetricsProps> = ({ senators, bills }) => {
  ...
  const averageMetrics = {
    attendance: senators.reduce((sum, s) => sum + s.stats.attendanceRate, 0) / senators.length,
    billsAuthored: senators.reduce((sum, s) => sum + s.stats.billsAuthored, 0) / senators.length,
    billsPassed: senators.reduce((sum, s) => sum + s.stats.billsPassed, 0) / senators.length,
    votingParticipation: senators.reduce((sum, s) => sum + s.stats.votingParticipation, 0) / senators.length
  };
  ...
  return (
    <div className="space-y-6">
          ...
          <table className="min-w-full">
            <thead>
              <tr className="border-b bg-gray-50">
                ...
                <th className="text-left py-3 px-4 text-sm font-medium text-gray-700">Senator</th>
                <th className="text-left py-3 px-4 text-sm font-medium text-gray-700">Party</th>
                <th className="text-center py-3 px-4 text-sm font-medium text-gray-700">Bills Passed</th>
                <th className="text-center py-3 px-4 text-sm font-medium text-gray-700">Bills Authored</th>
            ...
            <tbody>
              {topPerformers.map((senator, index) => (
                <tr key={senator.id} className={`border-b hover:bg-gray-50 ${index < 3 ? 'bg-yellow-50' : ''}`}>
                  ...
                  <td className="py-3 px-4">
                    <div>
                      <p className="font-medium text-gray-900">{senator.name}</p>
                      {senator.position && (
                        <p className="text-xs text-gray-500">{senator.position}</p>
                      )}
                    </div>
                  </td>
                  <td className="py-3 px-4">
                  ...
                  <td className="text-center py-3 px-4">
                    <span className="font-medium text-green-600">{senator.stats.billsPassed}</span>
                  </td>
                  <td className="text-center py-3 px-4">
                    <span className="font-medium">{senator.stats.billsAuthored}</span>
                  </td>
                  <td className="text-center py-3 px-4">
                      ...
                      <div className="w-16 bg-gray-200 rounded-full h-2 mr-2">
                        <div
                          className={`h-2 rounded-full ${
                            senator.stats.attendanceRate >= 90 ? 'bg-green-500' :
                            senator.stats.attendanceRate >= 75 ? 'bg-yellow-500' : 'bg-red-500'
                          }`}
                          style={{ width: `${senator.stats.attendanceRate}%` }}
                        />
                      </div>
                      <span className="text-xs">{senator.stats.attendanceRate}%</span>
                    </div>
                  </td>
                  <td className="text-center py-3 px-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            {[
              ...
              <div key={index} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                <div>
                  ...
                  <p className="text-xs text-gray-500">{committee.meetings} meetings held</p>
                </div>
                <div className="text-right">
                  <p className="font-bold text-primary-600">{committee.bills}</p>
        ...
        <Card className="p-6">
          <h3 className="text-lg font-semibold mb-4">Bill Success Rate by Type</h3>
