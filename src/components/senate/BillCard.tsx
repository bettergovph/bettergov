...
import { FileText, Clock, Users, ChevronRight, AlertCircle } from 'lucide-react';
        ...
        <div className="flex items-center justify-between p-3 border rounded-lg hover:bg-gray-50 transition-colors cursor-pointer">
          <div className="flex items-center gap-3 flex-1">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-gray-600 line-clamp-1">{bill.shortTitle || bill.title}</p>
            </div>
          </div>
        ...
        <div className="p-6">
            ...
            <div className="flex items-start gap-3 flex-1">
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-1">
                  <h3 className="text-lg font-semibold text-gray-900">{bill.number}</h3>
                  {getUrgencyIcon(bill.urgency)}
                </div>
                <h4 className="text-gray-700 font-medium">{bill.shortTitle || bill.title}</h4>
              </div>
            </div>
          ...
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-sm text-gray-600">
              By: <span className="font-medium text-gray-900">{bill.authors.join(', ')}</span>
              {bill.coAuthors.length > 0 && (
                <span className="text-gray-500"> (+{bill.coAuthors.length} co-authors)</span>
              )}
            </span>
          </div>
            ...
            <div className="border-t pt-3">
              <div className="flex items-center justify-between text-sm">
                <div className="flex items-center gap-2 text-gray-600">
                  ...
                  <span>Filed: {new Date(bill.dateField).toLocaleDateString()}</span>
                </div>
              ...
              <div className="mt-2">
                <div className="flex gap-1">
                  {['Filed', 'Committee', 'Second Reading', 'Third Reading', 'Transmitted'].map((stage, idx) => {
          ...
          {bill.committee && (
            <div className="mt-3 pt-3 border-t">
              <p className="text-xs text-gray-500">
                Assigned to: <span className="font-medium text-gray-700">{bill.committee}</span>
              </p>
            </div>
          )}
        </div>
