...
const VotingChart: React.FC<VotingChartProps> = ({ senators, bills }) => {
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card className="p-6">
          <h3 className="text-lg font-semibold mb-4">Voting Outcomes Distribution</h3>
        ...
        {/* Party Line Voting */}
        <Card className="p-6">
          <h3 className="text-lg font-semibold mb-4">Party Line Voting Alignment</h3>
          <ResponsiveContainer width="100%" height={300}>
      ...
      <Card className="p-6">
        <h3 className="text-lg font-semibold mb-4">Senator Voting Participation</h3>
        <div className="overflow-x-auto">
          <table className="min-w-full">
            <thead>
              <tr className="border-b">
                <th className="text-left py-2 px-3 text-sm font-medium text-gray-700">Senator</th>
                <th className="text-center py-2 px-3 text-sm font-medium text-gray-700">Yes Votes</th>
                <th className="text-center py-2 px-3 text-sm font-medium text-gray-700">No Votes</th>
                <th className="text-center py-2 px-3 text-sm font-medium text-gray-700">Abstentions</th>
                <th className="text-center py-2 px-3 text-sm font-medium text-gray-700">Absent</th>
                <th className="text-center py-2 px-3 text-sm font-medium text-gray-700">Participation</th>
              </tr>
            </thead>
            <tbody>
              {senators.slice(0, 5).map(senator => (
                <tr key={senator.id} className="border-b hover:bg-gray-50">
                  <td className="py-2 px-3 text-sm font-medium">{senator.name}</td>
                  ...
                  <td className="text-center py-2 px-3 text-sm">
                    <span className="text-gray-600 font-medium">
                      {Math.floor((100 - senator.stats.votingParticipation) * 0.9)}
                    </span>
                  </td>
                  <td className="text-center py-2 px-3 text-sm">
                    <div className="flex items-center justify-center">
                      <div className="w-16 bg-gray-200 rounded-full h-2 mr-2">
                        <div
                          ...
                          style={{ width: `${senator.stats.votingParticipation}%` }}
                        />
                      </div>
