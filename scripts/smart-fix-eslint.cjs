...
function fixReactImports(errors) {
  ...
  errors.reactImports.forEach(error => {
    ...
    if (match) {
      ...
      if (firstImport) {
        content = content.replace(firstImport[0], `import React from 'react';\n${firstImport[0]}`);
      } else {
    ...
    fileErrors.forEach(error => {
      ...
      if (lineIndex >= 0 && lineIndex < lines.length) {
        ...
        if (fixedLine.trim().match(/^[^<>]+</)) {
          fixedLine = fixedLine.replace(/^(\s*)([^<]+)/, (match, indent, text) => {
...
function removeUnusedImports(errors) {
  ...
  Object.entries(errorsByFile).forEach(([file, unusedVars]) => {
    ...
    fs.writeFileSync(filePath, content);
    console.log(`  ✓ ${file} - Removed unused imports: ${unusedVars.join(', ')}`);
  });
}
...
function main() {
  ...
  try {
    ...
    console.log(`  • React imports needed: ${errors.reactImports.length} files`);
    console.log(`  • Unescaped entities: ${errors.unescapedEntities.length} occurrences`);
