    ...
    fileErrors.forEach(error => {
      ...
      if (lineIndex >= 0 && lineIndex < lines.length) {
        ...
        if (colIndex >= 0 && colIndex < line.length) {
          ...
          const after = line.substring(colIndex + 1, Math.min(line.length, colIndex + 20));
          ...
          const isInJSXText = (before.lastIndexOf('>') > before.lastIndexOf('=')) || 
                              (after.indexOf('<') !== -1 && after.indexOf('<') < after.indexOf('='));
          
          if (!isInAttribute || isInJSXText) {
            // Replace the character at the specific position
            if (char === "'") {
              newLine = line.substring(0, colIndex) + '&apos;' + line.substring(colIndex + 1);
            } else if (char === '"') {
              newLine = line.substring(0, colIndex) + '&quot;' + line.substring(colIndex + 1);
            } else if (char === '>') {
              newLine = line.substring(0, colIndex) + '&gt;' + line.substring(colIndex + 1);
            ...
            if (newLine !== line) {
              ...
              modified = true;
              console.log(`  ✓ Fixed '${char}' at ${path.basename(filePath)}:${error.line}:${error.column}`);
            }
          } else {
            console.log(`  ⏭️  Skipped '${char}' in attribute at ${path.basename(filePath)}:${error.line}:${error.column}`);
          }
        }
      }
    });
